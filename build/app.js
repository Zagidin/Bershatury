	...
	*/
	...
	scene("start", () => {		
		...
		add(["startButton", sprite('screenWelcomeBtn', {
			width: btnWidth,
			height: btnHeight
		}), pos(gameWidth / 2 - (btnWidth * scaleCoef) / 2, 778 * scaleCoef), scale(scaleCoef), area()]);
	...
	scene("game", (selectedCharacter) => {		
		...
		const map = addLevel([
			...
			tiles: {
				...
				/*--- BONUSES ---*/
				A: () => ['bonus-shield', sprite('bonus-shield', {width: 22 * scaleCoef, height: 22 * scaleCoef}), area(), pos(tileSize / 8,tileSize / 4 - 1)],
				P: () => ['bonus-health', sprite('bonus-maxHealth', {width: 22 * scaleCoef, height: 22 * scaleCoef}), area(), pos(tileSize / 8,tileSize / 4 - 1)],
				R: () => ['bonus-double', sprite('bonus-double', {width: 22 * scaleCoef, height: 22 * scaleCoef}), area(), pos(tileSize / 8,tileSize / 4 - 1)],
				/*--- DANGER ---*/
				...
				C: () => ['danger', sprite('wires-1', {width: tileSize, flipX: randi()}), area({shape: new Rect(vec2(0), tileSize * 0.5, 20 * scaleCoef), offset: vec2(0, -1)}), anchor("center"), pos(vec2( (tileSize / 2) + randi(-3, 3),tileSize + 2)), z(1500)],
				//wires2: () => ['danger', sprite('wires-2', {width: tileSize / 3, flipX: randi()}), area(), anchor("bot"), pos(vec2((tileSize / 2) + randi(-4, 4),tileSize + 2)), z(-1)],
				...
				X: () => [
					'interactable', 
					sprite('workstation-1-broken', {width: 60 * scaleCoef}), 
					area(), 
					anchor("botleft"), 
					pos(0, tileSize + 3), 
					{
				...
				Y: () => [
					'interactable', 
					sprite('workstation-2-broken', {width: 60 * scaleCoef}), 
					area(), 
					anchor("botleft"), 
					pos(0, tileSize + 7), 
					{
				...
				Z: () => [
					'interactable', 
					sprite('workstation-3-broken', {width: 60 * scaleCoef}), 
					area(), 
					anchor("botleft"), 
					pos(0, tileSize + 7), 
					{
		...
		map.children.forEach(tile => {
			if(tile.is('interactable')) {
				tile.add(['sign', sprite('repair-sign', {width: 35 * scaleCoef}), anchor('top'), z(2000), pos(30 * scaleCoef, 5)]);
				tile.add(['text', sprite('repair-sign-2', {height: 10 * scaleCoef}), anchor('top'), z(2000), pos(30 * scaleCoef, -50)]);
				//tile.add(['text', text('ЗАПУСТИ', {font: 'Inter', size: 12}), color(Color.fromHex('#ff7d04')), anchor('bot'), pos(30 * scaleCoef, -40)]);
				tile.add(['repair-cloud', sprite('repair-cloud', {width: 120 * scaleCoef}), z(2000), pos(-27 * scaleCoef, -75 * scaleCoef)]).hidden = true;
				tile.add([
					'progress-bar',
					z(2500),
					pos(13 * scaleCoef, -30 * scaleCoef),
					{
						boxWidth: 40 * scaleCoef,
						boxHeight: 6 * scaleCoef,
						boxRadius: 1.5
					}
				]).hidden = true;
				...
				tile.add(['effect', sprite('effect-scull', {width: 7}), anchor('center'), pos(vec2(0, -35 * scaleCoef))]);
				...
				tile.add([
					sprite('arc-ball', {width: 20 * scaleCoef}),
					anchor('center'),
					pos(vec2(0, -35))
				]);
				tile.add([
				...
				tile.add([
					'arc-effect',
					sprite('arc-effect', {height: 65 * scaleCoef}),
					anchor('center'),
					pos(vec2(0,0)),
					area()
				]).hidden = true;
				tile.add(['effect', sprite('effect-scull', {width: 7}), anchor('center'), pos(vec2(0, -50 * scaleCoef))]);
				...
				tile.add(['effect', sprite('effect-scull', {width: 7}), anchor('center'), pos(vec2(0, -30 * scaleCoef))]);
			} else if(tile.is('win')) {
		...
		// frontal sprite
		//let foreground = add([sprite('background-2', {width: gameWidth}), anchor('botleft'), pos(vec2(0, gameHeight * 1.1)), fixed()]);
		
		// INTERFACE
		
		add([
			'btnTutorialOpen',
			sprite('screenTutorialBtnOpen', {width: 90}),
			anchor('center'),
			area(),
			fixed(),
			pos(gameWidth * 0.075, gameHeight * 0.9),
			z(2000)
		]);
		/* const tutorialScreen = add(['tutorialInGame', sprite('screenTutorialContent', {height: gameHeight}), fixed(), anchor('top'), pos(gameWidth / 2, 0), z(3000)]);
		tutorialScreen.hidden = true;
		tutorialScreen.add([rect(300, 300), color(Color.BLACK), opacity(0.3), pos(gameWidth / 2, 0), z(2900)]); */
		...
		tutorialScreen.add([
			'btnClose',
			sprite('screenTutorialBtnClose', {width: 90}),
			anchor('center'),
			area(),
			fixed(),
			...
			for(let i = 1; i <= player.maxHealth; i++) {
				if(i <= player.hp()) {
					drawSprite({
						...
						fixed: true,
						pos: vec2(gameWidth * 0.025 + (heartWidth + heartGap) * scaleCoef * i, interfaceTextPosY)
					});
				} else {
					drawSprite({
						sprite: "heart-empty",
				...
				if(player.isColliding(obj)) {
					if(obj.isInteractable && !obj.isRepaired && !player.isBlocked) {
						...
						obj.repairTimer.action = () => {
							obj.use(sprite(obj.spriteRepaired, {width: 60 * scaleCoef}));
		...
		/*
		let partScore = 5;
		let completeScore = 20;
		player.onCollide('computer-keyboard-collectible', (obj, e) => {
			let addScore = player.isDouble() ? partScore * 2 : partScore;
			player.score += addScore;
			if(!player.collectedParts.isComplete()) { // проверили первый раз перед сбором части
				if(!player.collectedParts.keyboard) player.collectedParts.keyboard = true;
				if(player.collectedParts.isComplete()) { // проверили второй раз после сбора, чтобы второй раз не давать очки
					let score = player.isDouble() ? completeScore * 2 : completeScore;
					player.score += score;
			...
			effect.use(pos(player.screenPos().x, player.screenPos().y));
			if(!player.collectedParts.isComplete()) {
				effect.use(sprite('computer-keyboard-collectible', {width: 55}));
				effect.get('effect-value')[0].text = '';
			} else {
				effect.use(sprite('score', {width: 50}));
				effect.get('effect-value')[0].text = '+' + addScore;
			}
			wait(effect.ttl, () => {
				effect.free = true;
				effect.hidden = true;
		...
		player.onCollide('computer-monitor-collectible', (obj, e) => {
			player.score += player.isDouble() ? partScore * 2 : partScore;
			if(!player.collectedParts.isComplete()) { // проверили первый раз перед сбором части
			...
			effect.use(pos(player.screenPos().x, player.screenPos().y));
			if(!player.collectedParts.isComplete()) {
				effect.use(sprite('computer-monitor-collectible', {width: 55}));
				effect.get('effect-value')[0].text = '';
			} else {
				effect.use(sprite('score', {width: 50}));
				effect.get('effect-value')[0].text = '+' + addScore;
		...
		player.onCollide('computer-mouse-collectible', (obj, e) => {
			let addScore = player.isDouble() ? partScore * 2 : partScore;
			player.score += addScore;
			if(!player.collectedParts.isComplete()) { // проверили первый раз перед сбором части
			...
			effect.use(pos(player.screenPos().x, player.screenPos().y));
			if(!player.collectedParts.isComplete()) {
				effect.use(sprite('computer-mouse-collectible', {width: 55}));
				effect.get('effect-value')[0].text = '';
			} else {
				effect.use(sprite('score', {width: 50}));
				effect.get('effect-value')[0].text = '+' + addScore;
		...
		player.onCollide('computer-case', (obj, e) => {
			let addScore = player.isDouble() ? partScore * 2 : partScore;
			player.score += addScore;
			if(!player.collectedParts.isComplete()) { // проверили первый раз перед сбором части
			...
			effect.use(pos(player.screenPos().x, player.screenPos().y));
			if(!player.collectedParts.isComplete()) {
				effect.use(sprite('computer-case', {width: 55}));
				effect.get('effect-value')[0].text = '';
			} else {
				effect.use(sprite('score', {width: 50}));
				effect.get('effect-value')[0].text = '+' + addScore;
		...
		*/
		...
		onUpdate(() => {
			...
			// if player falls between tiles, kill him and restart
			if(player.pos.y > 10000) {
				//player.use(playerSpawnPos);
...
function initAssets() {
	...
	// background
	loadSprite('background-0', 'assets/img/background/background-0.png');
	loadSprite('background-1', 'assets/img/background/background-1.png');
	loadSprite('background-2', 'assets/img/background/background-2.png'); // передний план
	
	loadSprite('cloud', 'assets/img/environment/cloud.png');
	
	//welcome screen sprites
	loadSprite('screenWelcomeLogo', 'assets/img/screens/welcome/logo.png');
	loadSprite('screenWelcomeCaption', 'assets/img/screens/welcome/caption.png');
	loadSprite('screenWelcomeBtn', 'assets/img/screens/welcome/btn_v2.png');
	
	//tutorial
	loadSprite('screenTutorialContent', 'assets/img/screens/tutorial/tutorial_complete.png');
	loadSprite('screenTutorialBtnContinue', 'assets/img/screens/tutorial/btn_continue.png');
	loadSprite('screenTutorialBtnClose', 'assets/img/screens/tutorial/btn_close.png');
	loadSprite('screenTutorialBtnOpen', 'assets/img/screens/tutorial/btn_open.png');
	
	//select screen sprites
	loadSprite('screenSelectLogo', 'assets/img/screens/select/logo_small.png');
	loadSprite('screenSelectBtnPrevNext', 'assets/img/screens/select/btn_prev_next.png');
	loadSprite('screenSelectBtnStart', 'assets/img/screens/select/btn_select.png');
	
	// ground tiles
	loadSpriteAtlas('assets/img/environment/tileset_v2.png', {
	...
	// environment
	loadSprite('platform', 'assets/img/environment/platform.png');
	loadSprite('spikes_bottom', 'assets/img/environment/spikes_bottom.png');
	loadSprite('spikes_top', 'assets/img/environment/spikes_top.png');
	loadSprite('wires-1', 'assets/img/environment/wires_1.png');
	loadSprite('wires-2', 'assets/img/environment/wires_2.png');
	loadSprite('effect-scull', 'assets/img/effects/danger_scull.png');
	loadSprite('arc-ball', 'assets/img/environment/arc_ball.png');
	loadSprite('arc-effect', 'assets/img/environment/arc_lightning.png');
	loadSprite('transformer', 'assets/img/environment/transformer.png');
	loadSprite('transformer-explode', 'assets/img/environment/explode.png', {
		sliceX: 4, sliceY: 1,
		anims: {'explode-anim': {from: 0, to: 3, loop: true}}
	});
	
	// characters
	loadSprite('char1_idle', 'assets/img/character/char_1_idle.png');
	loadSprite('char1_big', 'assets/img/character/char_1_big.png');
	loadSprite('char1_jump', 'assets/img/character/char_1_jump.png');
	loadSprite('char1_walk', 'assets/img/character/char_1_walk.png', {
		sliceX: 4, sliceY: 1,
		anims: {'char1-walk-anim': {from: 0, to: 3, loop: true}}
	});
	
	loadSprite('char2_idle', 'assets/img/character/char_2_idle.png');
	loadSprite('char2_big', 'assets/img/character/char_2_big.png');
	loadSprite('char2_jump', 'assets/img/character/char_2_jump.png');
	loadSprite('char2_walk', 'assets/img/character/char_2_walk.png', {
		sliceX: 4, sliceY: 1,
		anims: {'char2-walk-anim': {from: 0, to: 3, loop: true}}
	});
	
	loadSprite('char3_idle', 'assets/img/character/char_3_idle.png');
	loadSprite('char3_big', 'assets/img/character/char_3_big.png');
	loadSprite('char3_jump', 'assets/img/character/char_3_jump.png');
	loadSprite('char3_walk', 'assets/img/character/char_3_walk.png', {
		sliceX: 4, sliceY: 1,
		anims: {'char3-walk-anim': {from: 0, to: 3, loop: true}}
	});
	
	loadSprite('foreground', 'assets/img/background/foreground_.png');
	
	// objects
	loadSprite('score', 'assets/img/objects/score.png');
	loadSprite('heart', 'assets/img/objects/heart.png');
	loadSprite('heart-outline', 'assets/img/objects/heart_outline.png');
	loadSprite('heart-empty', 'assets/img/objects/heart_empty.png');
	loadSprite('bonus-maxHealth', 'assets/img/objects/bonus_max_health.png');
	loadSprite('bonus-double', 'assets/img/objects/bonus_double.png');
	loadSprite('bonus-shield', 'assets/img/objects/bonus_shield.png');
	
	loadSprite('computer-contour', 'assets/img/objects/computer_contour.png');
	loadSprite('computer-complete', 'assets/img/objects/computer_contour_complete.png');
	loadSprite('computer-keyboard', 'assets/img/objects/computer_keyboard.png');
	loadSprite('computer-keyboard-collectible', 'assets/img/objects/computer_keyboard_collectible.png');
	loadSprite('computer-monitor', 'assets/img/objects/computer_monitor.png');
	loadSprite('computer-monitor-collectible', 'assets/img/objects/computer_monitor_collectible.png');
	loadSprite('computer-mouse', 'assets/img/objects/computer_mouse.png');
	loadSprite('computer-mouse-collectible', 'assets/img/objects/computer_mouse_collectible.png');
	loadSprite('computer-case', 'assets/img/objects/computer_case.png');
	
	loadSprite('workstation-1', 'assets/img/objects/workstation_1.png');
	loadSprite('workstation-1-broken', 'assets/img/objects/workstation_1_broken.png');
	loadSprite('workstation-2', 'assets/img/objects/workstation_2.png');
	loadSprite('workstation-2-broken', 'assets/img/objects/workstation_2_broken.png');
	loadSprite('workstation-3', 'assets/img/objects/workstation_3.png');
	loadSprite('workstation-3-broken', 'assets/img/objects/workstation_3_broken.png');
	loadSprite('repair-sign', 'assets/img/effects/repair_sign.png');
	loadSprite('repair-sign-2', 'assets/img/effects/repair_sign_2.png');
	loadSprite('repair-text', 'assets/img/effects/repair_text.png');
	loadSprite('repair-cloud', 'assets/img/effects/repair_cloud_r.png', {
		sliceX: 4, sliceY: 1,
		anims: {'repair-anim': {from: 0, to: 3, loop: true}}
	});
	
	loadSprite('win-flag', 'assets/img/objects/win_flag.png');
	loadSprite('screen-win-flag', 'assets/img/screens/endgame/flag_win.png');
	loadSprite('screen-win-btn', 'assets/img/screens/endgame/btn_win.png');
	loadSprite('screen-lose-heart', 'assets/img/screens/endgame/heartbreak_lose.png');
	loadSprite('screen-lose-btn', 'assets/img/screens/endgame/btn_lose.png');
